...
def Main(argv: list[str], romWriter: Optional[RomWriter] = None) -> None:
    ...
    spoiler_file = open("spoilers/Sub"+logicChoice +
                        fillChoice+str(seeeed)+".sfc.spoiler.txt", "w")
    ...
    while not seedComplete:
        ...
        spoilerSave = ""
        spoilerSave += "Starting randomization attempt:" + \
            str(randomizeAttempts)+"\n"
        ...
        while len(unusedLocations) != 0 or len(availableLocations) != 0:
            ...
            if placeLocation == "Fail":
                print(
                    f'Item placement was not successful due to majors. {len(unusedLocations)} locations remaining.')
                spoilerSave += f'Item placement was not successful. {len(unusedLocations)} locations remaining.\n'
                break
    ...
    # initial game state set by $82:eeda
    romWriter.writeBytes(0x16eda, b"\x1f")
    romWriter.writeBytes(0x16ee0, b"\x06\x00")  # initial game area = 6 (ceres)
    romWriter.writeBytes(0x16ee3, b"\x9f\x07")  # $079f Area index
    # $7e:d914 = 05 Main
    romWriter.writeBytes(0x16ee5, b"\xa9\x05\x00\x8f\x14\xd9\x7e\xea\xea")
    # jsl save game (param in A: save slot)
    romWriter.writeBytes(0x16eee, b"\xad\x52\x09\x22\x00\x80\x81")
    # adjust earlier branch to go +6 bytes later to rts
    romWriter.writeBytes(0x16ed0, b"\x24")
    # adjust earlier branch to go +6 bytes later to rts
    romWriter.writeBytes(0x16ed8, b"\x1c")
    ...
    # set the carry bit (a lot)
    romWriter.writeBytes(0x23e33, b"\x38\x38\x38\x38")
    ...
    print("Spoiler file is "+"Sub"+logicChoice +
          fillChoice+str(seeeed)+".sfc.spoiler.txt")
